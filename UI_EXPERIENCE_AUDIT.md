...
| Play / polls / nominations | Partial | Brand typography, artwork and action layout are present. Physical small-screen, keyboard, animation and motion/accessibility checks remain. At the user's request, web-v71 deliberately replaces even iOS-style decorative streak emoji with vectors; see [UI_ASSET_PARITY.md](UI_ASSET_PARITY.md). |
...
| Memento review / sharing / history | Partial | The web-v72 candidate follows the active Swift flow: one shutter for sequential views, photo review with Retake / Send to the named chat, and visible send feedback. No caption, effects or extra-options form; photo selection is available only after camera failure. Secondary-upload release evidence is tracked in MEMENTO_SEND_FIX_2026-09-07.md. Date boundaries and real-account/device confirmation remain. |
