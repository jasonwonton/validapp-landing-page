...
test('unintercepted upload transport preserves signed headers and exact bytes', async ({ page }) => {
    const uploads = [];
    const sources = new Map(await Promise.all(['api.js', 'auth-route-recovery.js', 'auth-reliability.js', 'auth-diagnostics.js'].map(async name => [`/app/${name}`, await readFile(new URL(`../app/${name}`, import.meta.url))])));
