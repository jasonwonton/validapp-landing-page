...
async function seedVideos(page) {
    ...
    await page.evaluate(async () => {
        // A built release loads its modules from a hashed path.
        const manifest = await fetch("/app/build-manifest.json").then((response) => response.ok ? response.json() : null).catch(() => null);
        const { DemoAPI } = await import(manifest?.assets?.["/app/demo-api.js"] || "/app/demo-api.js");
