...
| Voice messages | Partial | Not yet tested | Inline composer microphone records compatible MP4 audio with a five-minute ceiling, preview, discard/re-record and send. Incompatible browsers open an M4A picker from that same control. Media no longer contains voice controls. Existing duration validation, private upload, playback and recovery remain; native hold/slide/lock, waveform UI and physical permission UX remain open. |
| Stickers placed on chat photos | Partial | Not yet tested | Review exposes native-style cutout and My Stickers actions. Up to eight saved stickers can be dragged, pinch-resized, removed, or positioned/resized with a keyboard, and are baked into the JPEG before the existing durable upload/send. Rotation, physical multitouch and final-device acceptance remain open. |
