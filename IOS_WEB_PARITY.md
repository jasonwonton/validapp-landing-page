...
September 7 Memento send correction: the additive secondary-upload API patch
and simplified `web-v72` capture/review are now live on private staging.
Real-origin contract, file-hash and synthetic camera checks pass; intended-account
physical-device sending remains an acceptance gate. See
[MEMENTO_SEND_FIX_2026-09-07.md](MEMENTO_SEND_FIX_2026-09-07.md).
...
| Voice messages | Partial | Not yet tested | Inline compatible MP4 recording supports hold/release-to-preview, slide-left cancellation, slide-up locking, measured waveform, custom play/seek, discard/re-record and explicit send. Tap/keyboard toggle and M4A fallback remain. Local audio is bounded to five minutes/4 MB; physical touch, microphone interruption and codec acceptance remain open. |
| Stickers placed on chat photos | Partial | Not yet tested | Native-style cutout/My Stickers actions and a consistent three-column picker with New tile. Up to eight saved stickers support drag, pinch/rotation, removal and keyboard equivalents; transforms are baked into the existing durable JPEG upload. Actual pixel/rotation tests pass; physical multitouch and final-device acceptance remain open. |
...
| Photo Effects / Featured filter catalog | Partial | Not yet tested | The active iOS camera ships bundled tracked lenses plus a bounded server-managed Featured catalog; AI creation is explicitly dormant. PWA Chat and Story photo composers provide five offline color/lighting Effects and browser-compatible treatments from at most 16 validated Featured recipes. Memento review intentionally omits the effects form to match the active Swift Memento journey. Results are baked into bounded JPEGs before private, idempotent upload. Physical camera/color review remains. |
...
| Voice/video calls | Partial | Not yet tested | Private staging permits initiation from the native-style phone button, with camera escalation inside the call, circular mute/camera/end controls and browser audio-output selection where supported. Existing LiveKit/control contracts remain; stale permission/connect/camera/event results cannot revive an ended call, ambiguous starts retain bounded retry identities. Four-project mocked checks pass; real two-account audio/video, device permissions, Bluetooth, backgrounding and network handoff remain open. Reliable closed-app/lock-screen ringing is native-only; this preview requires Six7 open. |
...
| Dark Mode | Partial | Not yet tested | Profile now offers persistent System/Light/Dark selection, including browser chrome and all lazy feature styles. Fixed white-on-pale profile/Feed/Play surfaces, dark secondary text, peach action contrast and school symbols. Regression tests measure sampled text pairs at 4.5:1 or better; exhaustive screen/state/text-size and physical accessibility acceptance remain. |
| Haptics | Partial | Not yet tested | Android tap and success feedback share bounded, throttled vibration with persistent opt-out and a Test vibration control. Unsupported/denied browsers degrade safely. Physical feedback depends on browser/device settings; native haptic fidelity is not promised. |
